...
function spread(vertices::Vector{Tuple{Int,Int,Int}})
    return map(function (v)
        (val, i, j) = v
        return (val, 2 * i - 1, 2 * j - 1)
    end, vertices)
end
...
- edgeMatrix: adjacency matrix with 1s for single bridges and 2s for double briges
- V: list of vertices
"""
function writeSolution(fout::IOStream, edgeMatrix::Array{Int, 2}, V::Vector{Tuple{Int,Int,Int}})

    println(fout, "edgeMatrix = [")
    m, n = size(edgeMatrix)
    
    for i = 1:m
        for j = 1:n
            print(fout, string(edgeMatrix[i, j]) * " ")
        end 
...
function displayGrid(grid::Array{Int, 2})
    ...
    for i = 1:m
        for j=1:n
            if grid[i,j] > 0
                print(" " * intToString(grid[i,j], cellWidth) * " ")
            else
...
function displaySolution(edgeMatrix::Array{Int,2}, vertices::Vector{Tuple{Int,Int,Int}})
    vertices = spread(vertices)
    m = maximum(map(function (v)
        (_, i, j) = v
        return i
    end, vertices))
    n = maximum(map(function (v)
        (_, i, j) = v
        return j
    end, vertices))
    ...
    for k1 = 1:K
        for k2 = 1:k1
            bridges = edgeMatrix[k1, k2] 
            if bridges > 0 
                (_, i1, j1) = vertices[k1]
                (_, i2, j2) = vertices[k2]
                if i1 == i2
                    bridgeStr = hBridgeChar[bridges]
                    loopRange = j1 + 1:j2 - 1
                    if j1 > j2
                        loopRange = j2 + 1:j1 - 1
                    end
